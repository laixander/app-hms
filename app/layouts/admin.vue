...
<template>
    <div :class="['min-h-screen bg-slate-950', { 'sidebar-collapsed': isCollapsed }]">
        <!-- Mobile Sidebar Overlay -->
        <div v-if="isSidebarOpen" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-40 lg:hidden"
        ...
        <div class="admin-content">
            <!-- Top Bar -->
            <header
                class="sticky top-0 z-30 bg-slate-950/80 backdrop-blur-lg border-b border-white/5 px-4 lg:px-8 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <button class="lg:hidden p-2 -ml-2 text-slate-400 hover:text-white" @click="isSidebarOpen = true">
                        <UIcon name="i-lucide-menu" class="w-6 h-6" />
                    </button>
                    <button class="hidden lg:block p-2 -ml-2 text-slate-400 hover:text-white"
                ...
                <div class="flex items-center gap-4">
                    ...
                    <button
                        class="relative w-9 h-9 rounded-lg bg-white/5 flex items-center justify-center hover:bg-white/10 transition-colors">
                        <UIcon name="i-lucide-bell" class="w-4 h-4 text-slate-400" />
                        <span
                            class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-slate-950" />
                    </button>
                    <NuxtLink to="/kiosk" class="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1">
